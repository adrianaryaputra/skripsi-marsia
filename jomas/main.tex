...
\subsection{Qualitative Trustworthiness and Rigour}
...
Table~\ref{tab:order-cycle-times} reveals that for GrabFood peak-hour orders, internal kitchen operations account for 12.0 minutes (50.0\% of total cycle time, split into 8.0 minutes for assembly and 4.0 minutes for packaging), whereas external platform and logistics latency contributes another 12.0 minutes (50.0\%, comprising 6.0 minutes of digital ingestion and driver arrival wait, plus 6.0 minutes of last-mile road transit). The peak lunch rush doubles kitchen assembly time from 4.0 to 8.0 minutes because kitchen staff must batch digital orders while concurrently serving visible dine-in patrons. Furthermore, the 4.0 minutes required for packaging separate condiment pouches under frantic physical conditions creates the exact operational strain that leads to omission errors.
...
\subsection{Linguistic and Valence Taxonomy of GrabFood Customer Grievances}
To understand the semantic mechanisms through which fulfillment failures degrade customer sentiment, we conducted an in-depth linguistic and sentiment valence audit across the repository of 4,584 GrabFood customer reviews. Customer reviews on food delivery platforms represent unstructured digital feedback where emotional valence directly correlates with star ratings. While high ratings (5 stars) are dominated by positive sensory descriptors such as \textit{"gurih"} (savory), \textit{"porsi besar"} (generous portion), and \textit{"empuk"} (tender), negative reviews display highly concentrated keyword clusters denoting operational fulfillment failures.

We systematically coded and extracted grievance terms into semantic clusters. For each keyword cluster, we evaluated its frequency of occurrence within critical reviews (ratings $\le 3$ stars), the mean rating penalty relative to the baseline store rating of 4.70, and the primary operational failure mechanism. Table~\ref{tab:linguistic-taxonomy} presents the resulting linguistic keyword taxonomy and valence impact.
...
\toprule
\textbf{Keyword Cluster (Indonesian / English)} & \textbf{Mentions} & \textbf{Mean Rating} & \textbf{Penalty} & \textbf{Underlying Operational Failure Mechanism} \\
\midrule
\textit{"kuah", "bumbu basah"} (gravy / curry sauce) & 312 & 1.42 / 5.0 & $-3.28$ & Curry gravy omitted from package; dry meal experience \\
\textit{"sambal", "lado mudo"} (chili / sambal cup) & 246 & 1.58 / 5.0 & $-3.12$ & Sambal pouch forgotten; loss of signature flavor profile \\
\textit{"lama", "nunggu", "antri"} (waiting time / delay) & 198 & 2.15 / 5.0 & $-2.55$ & Courier waits $>20$ min at counter; cold food upon delivery \\
\textit{"salah menu", "ketukar"} (swapped / wrong item) & 144 & 1.18 / 5.0 & $-3.52$ & Mismatched 4-digit POS ticket; wrong protein received \\
\textit{"dingin", "anyep"} (cold food / loss of heat) & 96 & 2.30 / 5.0 & $-2.40$ & Thermal dissipation from dispatch delay and transit time \\
\textit{"tumpah", "bocor"} (gravy spillage / leaking) & 82 & 1.74 / 5.0 & $-2.96$ & Single-knot plastic pouch ruptured inside carrier bag \\
\bottomrule
\end{tabularx}
\end{table}

The empirical findings in Table~\ref{tab:linguistic-taxonomy} reveal that wrong items and omitted condiments trigger the most severe valence penalties in customer reviews. Specifically, receiving a swapped menu variant incurs a devastating mean rating drop of $-3.52$ points (yielding an average rating of only 1.18 out of 5.0), while omitted curry gravy drops the rating by $-3.28$ points (to 1.42 out of 5.0). Notably, customers frequently pair these negative ratings with explicit statements emphasizing that the food itself tastes excellent, but the absence of the essential gravy pouch completely spoils the consumption encounter. This highlights how peripheral meal components carry disproportionate emotional weight in consumer evaluations.
...
3. Omission + Digital Store Voucher & 56,000 & 44,800 & 22,750 & 10,000 & +12,050 & 21.5\% & $-45.3\%$ \\
...
\subsection{Theoretical Deepening: Expectation-Disconfirmation and Asymmetric Attribute Performance}
...
\textcite{johnston1995determinants} classified service quality attributes into satisfiers and dissatisfiers (hygiene factors). Satisfiers are attributes that enhance satisfaction when fulfilled but do not cause intense dissatisfaction when absent. Conversely, dissatisfiers or hygiene factors are baseline operational attributes that customers take for granted; their presence merely prevents dissatisfaction, but their absence triggers acute negative disconfirmation. In Indonesian culinary culture, particularly for Padang cuisine, rice meals are fundamentally consumed with copious curry gravy (\textit{kuah gulai}) and sambal. While generous protein portions and authentic spice blends serve as core satisfiers, the inclusion of complete sauces and condiments functions as an absolute hygiene factor. 

When a restaurant successfully delivers a well-spiced rendang with complete gravy, customers experience neutral or mild positive confirmation, awarding 4 or 5 stars. However, when the kitchen omits the gravy pouch, severe negative disconfirmation occurs. The meal becomes dry and unpalatable, violating the baseline consumption premise. Under the asymmetric impact model of \textcite{mittal1998asymmetric}, negative performance on a hygiene attribute exerts a substantially greater psychological impact on overall satisfaction than an equivalent positive performance on a satisfier attribute. As documented in our empirical valence audit (Table~\ref{tab:linguistic-taxonomy}), the absence of a Rp1,100 condiment pouch destroys the customer's perceived value of an entire Rp56,000 meal package, resulting in an unrecoverable 1-star review.

This cognitive asymmetry is further exacerbated by the platform pricing mark-up. Because consumers pay a 24.4\% price premium over physical dine-in prices (Table~\ref{tab:price-markup}), their initial performance expectations are calibrated higher than in-store diners. At this elevated expectation threshold, customers exhibit zero tolerance for execution defects. Furthermore, unlike physical dining rooms where customers can immediately signal a missing sauce to passing waitstaff, digital delivery encounters are mediated by physical distance and third-party couriers. Once the driver departs, the customer faces high friction in obtaining redress, transforming operational oversights into acute psychological frustration.

\subsection{Hierarchical Digital Service Recovery Framework}
...
\begin{enumerate}[leftmargin=*,label=\textbf{\arabic*.},itemsep=4pt,topsep=4pt]
    ...
    Restaurant operators must recognize the cognitive asymmetry governing digital consumer evaluations (Oliver, 1980; Johnston, 1995). While kitchen staff often view side sauces, green chili cups, and curry gravy as low-cost incidental add-ons, digital delivery consumers treat these condiments as non-negotiable hygiene attributes. An omitted condiment pouch induces severe negative disconfirmation, destroying the perceived utility of an otherwise well-prepared premium meal and triggering devastating 1-star reviews (Table~\ref{tab:linguistic-taxonomy}). Management must instill an organizational culture where condiment completeness is treated with the same operational discipline as main protein cooking. Condiments should be pre-portioned, sealed, and inventoried during pre-rush staging rather than ladled on-the-fly during peak lunch velocity.
