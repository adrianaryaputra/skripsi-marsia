...
The empirical findings in Table~\ref{tab:linguistic-taxonomy} reveal that wrong items and omitted condiments trigger the most severe valence penalties in customer reviews. Specifically, receiving a swapped menu variant incurs a substantial mean rating drop of $-3.52$ points (yielding an average rating of only 1.18 out of 5.0), while omitted curry gravy drops the rating by $-3.28$ points (to 1.42 out of 5.0). In customer feedback, these negative ratings are frequently paired with explicit statements emphasizing that while food taste is satisfactory, omitting the required gravy pouch compromises the meal experience. This highlights how peripheral meal components carry disproportionate emotional weight in consumer evaluations.
